...
function approve() {
    ...
    approveBUSD(web3.utils.toWei(trxspenddoc.value.toString()));
}
...
function refreshData(){
    ...
    contractBalance(function(result){
        ...
        balanceElem.textContent = stripDecimals(rawStr, 2) + ' BUSD';
    });*/
    ...
    userTotalDeposits(function(result){
        ...
        userDepositElem.textContent = stripDecimals(rawStr, 3) + ' BUSD';
    });
    ...
    totalInvested(function (result) {
        rawStr = numberWithCommas(Number(result).toFixed(2));
        investedElem.textContent = stripDecimals(rawStr, 2) + ' BUSD';
    });
   ...
   // userDividends
    ...
    // userDividends(function(result){
        ...
        // userDividendsElem.textContent = stripDecimals(rawStr) + ' BUSD';
    // });

    var userAvailableElem = document.getElementById('user-available');
    userAvailable(function(result){
        rawStr = Number(result).toFixed(3);
        userAvailableElem.textContent = stripDecimals(rawStr) + ' BUSD';
    });
    ...
    // userTotalReferrals(function(result){
        // rawStr = Number(result).toFixed(3);
        // userTotalReferralsElem.textContent = rawStr.toLocaleString() + " BUSD";
    // });

    var spentLimitElem = document.getElementById('spend-limit');
    spendLimit(function(result){
        rawStr = Number(result).toFixed(3);
        spentLimitElem.textContent = rawStr.toLocaleString() + " BUSD";
    });

    var userBalanceElem = document.getElementById('user-balance');
    userBalance(function(result){
        rawStr = Number(result).toFixed(3);
        userBalanceElem.textContent = rawStr.toLocaleString() + " BUSD";
    });
	...
	userDepositInfo(plan-1, function(result){
		if(result.amount){
			...
			myInvestPlan1Elem.textContent = "Balance: " + rawStr.toLocaleString() + " BUSD";
		}
    });
	...
	userDepositInfo(plan-1, function(result){
		if(result.amount){
			var amount = web3.utils.fromWei(result.amount);
			rawStr = Number(amount).toFixed(3);
			myInvestPlan2Elem.textContent = "Balance: " + rawStr.toLocaleString() + " BUSD";
		}
    });
	...
	userDepositInfo(plan-1, function(result){
		if(result.amount){
			var amount = web3.utils.fromWei(result.amount);
			rawStr = Number(amount).toFixed(3);
			myInvestPlan3Elem.textContent = "Balance: " + rawStr.toLocaleString() + " BUSD";
		}
    });
	...
	userDepositInfo(plan-1, function(result){
		if(result.amount){
			var amount = web3.utils.fromWei(result.amount);
			rawStr = Number(amount).toFixed(3);
			myInvestPlan4Elem.textContent = "Balance: " + rawStr.toLocaleString() + " BUSD";
		}
    });
